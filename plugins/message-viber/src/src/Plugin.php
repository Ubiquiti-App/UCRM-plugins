...
namespace ViberNotifier;


use Psr\Log\LogLevel;
use ViberNotifier\Facade\ViberNotifierFacade;
use ViberNotifier\Factory\NotificationDataFactory;
use ViberNotifier\Service\OptionsManager;
use ViberNotifier\Service\PluginDataValidator;
...
class Plugin
{
    ...
    public function __construct( Logger $logger, OptionsManager $optionsManager, PluginDataValidator $pluginDataValidator, ViberNotifierFacade $notifierFacade, NotificationDataFactory $notificationDataFactory )
    {
        $this->logger = $logger;
    ...
    public function run(): void
    {
        if (PHP_SAPI === 'fpm-fcgi') {
            $this->logger->info('Viber messages over HTTP started');
            $this->processHttpRequest();
            $this->logger->info('HTTP request processing ended.');
        } elseif (PHP_SAPI === 'cli') {
            $this->logger->info('Viber messages over CLI started');
            $this->processCli();
