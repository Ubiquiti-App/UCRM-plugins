...
$orgSelected = 0;
    ...
    if (!file_exists($this->path.$CERT)) $this->error .= " Failed to open ".$CERT;  else if(DEBUG) echo 'cert ok <br>';
    if (!file_exists($this->path.$PRIVATEKEY)) $this->error .= " Failed to open ".$PRIVATEKEY; else if(DEBUG) echo 'privatekey ok <br>';
    if (!file_exists($this->path.$WSDL)) $this->error .= " Failed to open ".$WSDL; else if(DEBUG) echo 'wsdl ok <br>';
  ...
  private function sign_TRA($orgSelected)
  {
	...
	$STATUS = openssl_pkcs7_sign(realpath("classes/Argentinacae/xmlgenerados/TRA" .$orgSelected . ".xml"), realpath("classes/Argentinacae/xmlgenerados/TRA".$orgSelected.".tmp"), "file://".realpath($CERT),
      array("file://".realpath($PRIVATEKEY), self::PASSPHRASE),
      array(),
      !PKCS7_DETACHED
    );
