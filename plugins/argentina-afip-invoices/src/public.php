...
foreach (explode(';', $config['salesPoint']) as $organizacion) {
    ...
    if (! (is_bool($organizationKey) && $organizationKey === false)) {
        $organizaciones[$count] = ['name' => $organizacionesSinKey[$count][0], 'salesPoint' => $organizacionesSinKey[$count][1], 'activitiesStartDate' => $organizacionesSinKey[$count][2], 'id' => $organizations[$organizationKey]['id']];
    }
    ++$count;
}
if (DEBUG) {
